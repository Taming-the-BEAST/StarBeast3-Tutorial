...
This tutorial is heavily based on the tutorial for StarBEAST2 by Huw A. Ogilvie TODO:cite.

TODO: To avoid repetition, you should read through all parts even if interested in a specific kind of analysis.

# Dataset

In this tutorial we will reconstruct species tree of **Canis** genus. We will analyse 24 species, 8 out which are extant. We will use 16 nuclear loci (TODO cite) for extant taxa in the first two analyses. We will add 50 morphological character data for all 24 taxa for the total evidence analysis. Before starting download all the data for this tutorial (TODO make a download zip). You may browse these data using your preferred alignment viewer before proceeding with the tutorial.
...
### TreeAnnotator (TODO, probably ugly trees)
...
<a id="fig1"></a> <img src="figures/install.jpeg" style="width:70.0%;"/>
...
You should now see that every species has two haplotypes assigned to it as in Figure TODO.
...
>
> For **Frequencies**, select **empirical** from the dropdown list
>
> Now, on the partition list to the left, select all partitions except the first one.
>
> At the **Clone from** drop down list, choose the first partition that you have set up.
...
In this analysis we will estimate relative clock rate for each gene and set the species tree clock rate to 0.001 (TODO based on ..). Note, that in real analyses you can rarely avoid estimating the clock rate as it required a lot of data. Using strict clock is similarly too simplistic for many cases.
...
>
> For **Tree.t:Species** select **Birth Death Model**.
>
...
> Launch **Tracer**
>
> Drag and drop *canid_strict_clock.log* on the **Tracer** window.
...
Use settings as in figure TODO to produce MCC tree. If you are confused about the set up, please refer to [Introduction to BEAST2](https://taming-the-beast.org/tutorials/Introduction-to-BEAST2/) tutorial.

### Produce CCD Tree

Use settings as in figure TODO to produce CCD tree. If you are confused about the set up, please refer to [Introduction to BEAST2](https://taming-the-beast.org/tutorials/Introduction-to-BEAST2/) tutorial.
...
Now your figure should look similar to Figure TODO.
...
>
> Set **Clock.rate** to 0.001 TODO fig
...
>
> Assign a _LogNormal(M=0.001, S=1)_ prior to **SpeciesTreeRelaxedClockRate.Species**. Ensure that **Mean in Real Space** is cheked. TODO fig
...
Scroll down in the parameter list, you should notice new parameters describing the empirical mean, variance, coefficient of variation of the estimated per-branch rates as well as the standard deviation parameter to the uncorrelated relaxed clock of the species tree. TODO fig
...
The taxon set is set up the same way as before (TODO:pic).
...
> Switch to **Priors** tab
> 
> From drop down list, select **FBDModel** TODO:pic

Now, we can add the morphological data. You will be prompted to decide if Lewis MK or MKv model should be used. 
Our data contains only variable characters, therefore we will use MKv model (see more at TODO).
...
Morphological data should now appear within the **Partitions** tab TODO:fig.
...
The tab should now look like figure TODO.
...
For morphological data, we will not change the default model. Select one of the morphological partitions and verify that the **Subst Model** is **Lewis MK** (ascertainment correction will not be shown here) and it looks like figure TODO.
...
In the very first step we have already selected **FBDModel** as species tree prior. Note, that we do not need to change the default settings for this analysis. However, if you need to change or estimate sampling at present probability or conditioning, you must do it here. You can find more information on how to do that in the optional section below. TODO
...
>
> Set M=0.001 and S=1.0 and check the **Mean In Real Space** checkmark. Figure TODO

Now we will set the prior for strict clock rate of morphological data:

> Navigate to **clockRate.c:morphology_canis** parameter
>
> Select **1/X** from the drop down list. Figure TODO 
...
As before, you may open and explore the trace _.log_ file in Tracer, uglytrees.com and icytree.org. See sections 1.10 through 1.12. The section 1.13 guides you on how to make a summary tree. Go ahead and make one for the FBD analysis. 

Now, load the trees from section 1 and section 3 in FBD and compare. 

> Question: Can you identify any diferences in clade support? 

You should find that the tree topology is largely similar. However, the species _Cuon alpinus_ and _Lycaon pictus_ are now completely supported in by the FBD analysis, while it is not the case when we use only molecular data in section 1. This discrepancy has been previously reported (TODO:cite).  
...
TODO: To facilitate easy use of uglytrees, the species and individual names should have a similar pattern so that uglytrees can guess the matching automatically, e.g., individual names should contain the species name as a substring, or before the first \_ etc.
